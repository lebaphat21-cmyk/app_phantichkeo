...
with tab5:
    ...
    _ytd_opts = sorted(df['year'].unique())
    sel_yr_5 = st.selectbox("Chọn năm xem YTD:", options=_ytd_opts, index=len(_ytd_opts)-1, key='ytd_yr')
    ...
    _wk_opts = sorted(df['year'].unique())
    sel_yr_w = st.selectbox("Chọn năm:", _wk_opts, index=len(_wk_opts)-1, key='wk_yr')
